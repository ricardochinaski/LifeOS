...
export const DashboardViewV2: React.FC = () => {
  ...
  const rawDateLabel = dateOnlyToLocalDate(today).toLocaleDateString('es-CL', {
    weekday: 'long',
    day: 'numeric',
    month: 'long',
  });
  ...
  const latestHealthLog = useMemo(
    () => [...healthLogs]
      .filter((log) => !isDemoHealthLog(log))
    ...
    <div className="mx-auto max-w-6xl space-y-3 pb-[calc(6rem+env(safe-area-inset-bottom,0px))] animate-fade-in sm:space-y-5">
      <header className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 text-white shadow-xl">
        <div className="p-4 sm:p-5">
          ...
          <div className={`mt-3 flex items-center gap-2 rounded-xl px-3 py-2 ${plan.overdueCount > 0 ? 'bg-rose-500/10 text-rose-200' : 'bg-emerald-500/10 text-emerald-200'}`}>
            ...
            <p className="min-w-0 text-xs"><span className="font-black">{contextTitle}</span> · {contextSummary}</p>
            <button onClick={() => setActiveTab('tasks')} className="ml-auto shrink-0 text-[10px] font-black text-white/80">Tareas</button>
          </div>
        </div>

        <div className="grid grid-cols-4 border-t border-slate-800 bg-slate-900/80 text-center">
          <div className="p-2.5">
            <p className="text-[9px] font-bold uppercase text-slate-500">Atrasadas</p>
            <p className={`mt-0.5 text-lg font-black ${plan.overdueCount ? 'text-rose-400' : 'text-white'}`}>{plan.overdueCount}</p>
          </div>
          <div className="border-l border-slate-800 p-2.5">
            <p className="text-[9px] font-bold uppercase text-slate-500">Hoy</p>
            <p className="mt-0.5 text-lg font-black text-amber-300">{plan.todayCount}</p>
          </div>
          <div className="border-l border-slate-800 p-2.5">
            <p className="text-[9px] font-bold uppercase text-slate-500">Hábitos</p>
            <p className="mt-0.5 text-lg font-black text-emerald-300">{plan.habitsCompleted}/{plan.dueHabits.length}</p>
          </div>
          <div className="border-l border-slate-800 p-2.5">
            <p className="text-[9px] font-bold uppercase text-slate-500">Entreno</p>
            <p className="mt-0.5 text-lg font-black text-sky-300">{workoutMinutesToday} min</p>
          </div>
        </div>
      </header>
      ...
      <section className="grid gap-3 lg:grid-cols-[1.45fr_0.85fr]">
        <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <p className="text-[9px] font-black uppercase tracking-[0.16em] text-emerald-500">Prioridad ejecutable</p>
          ...
          <div className="space-y-2">
            {plan.focusTasks.length === 0 ? (
              <div className="rounded-xl border border-dashed border-emerald-300 bg-emerald-50/60 p-5 text-center dark:border-emerald-900 dark:bg-emerald-950/20">
              ...
              return (
                <div key={task.id} className={`flex items-start gap-3 rounded-xl border p-3 ${overdue ? 'border-rose-200 bg-rose-50/70 dark:border-rose-900 dark:bg-rose-950/20' : 'border-slate-200 bg-slate-50/70 dark:border-slate-800 dark:bg-slate-800/40'}`}>
        ...
        <div className="space-y-3">
          <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-4">
            ...
            <div className="space-y-1.5">
              {plan.dueHabits.length === 0 ? (
                <p className="rounded-xl bg-slate-50 p-3 text-xs text-slate-500 dark:bg-slate-800/50">No hay hábitos programados.</p>
              ) : plan.dueHabits.slice(0, 5).map((habit) => {
                const done = plan.completedHabitIds.has(habit.id);
                return (
                  <button key={habit.id} onClick={() => logHabit(habit.id)} className={`flex min-h-10 w-full items-center gap-3 rounded-xl border px-3 py-2 text-left transition ${done ? 'border-emerald-200 bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950/30' : 'border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-800/40'}`}>
                    {done ? <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-500" /> : <Circle className="h-4 w-4 shrink-0 text-slate-400" />}
          ...
          {(syncHasError || !currentUser || syncIsBusy) && (
            <div className={`rounded-2xl border p-3 shadow-sm ${syncHasError ? 'border-rose-200 bg-rose-50 dark:border-rose-900 dark:bg-rose-950/30' : 'border-slate-800 bg-slate-900 text-white'}`}>
              <div className="flex items-center gap-2">
              ...
              <p className={`mt-1 text-xs font-bold ${syncHasError ? 'text-rose-700 dark:text-rose-300' : 'text-slate-200'}`}>
              ...
              {lastSyncedAt && <p className={`mt-1 text-[10px] ${syncHasError ? 'text-rose-500' : 'text-slate-500'}`}>Último sync: {new Date(lastSyncedAt).toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit', hour12: false })}</p>}
            </div>
          )}
        </div>
      </section>
      ...
      <details className="group rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-3 sm:p-4">
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-400">Divulgación progresiva</p>
        ...
        <div className="grid grid-cols-2 gap-3 border-t border-slate-200 p-3 dark:border-slate-800 sm:grid-cols-4 sm:p-4">
          <button onClick={() => setActiveTab('finances')} className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-left dark:border-slate-800 dark:bg-slate-950/40">
            ...
            <p className="mt-1 text-[10px] text-slate-500">{budgetPct === null ? 'Sin presupuesto real' : `${budgetPct}% del presupuesto`}</p>
          </button>
          ...
          <button onClick={() => setActiveTab('health')} className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-left dark:border-slate-800 dark:bg-slate-950/40">
            ...
            <p className="mt-1 text-sm font-black text-slate-900 dark:text-white">{latestHealthLog?.spO2Pct ? `SpO₂ ${latestHealthLog.spO2Pct}%` : 'Sin biometría real'}</p>
          ...
          <button onClick={() => setActiveTab('library')} className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-left dark:border-slate-800 dark:bg-slate-950/40">
            ...
            <p className="mt-1 truncate text-sm font-black text-slate-900 dark:text-white">{activeBook?.title || 'Sin lectura real activa'}</p>
      ...
      <details className="group rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-3 sm:p-4">
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-400">Herramientas secundarias</p>
            <p className="text-sm font-black text-slate-900 dark:text-white">Turno y temporizador de foco</p>
          </div>
          <ChevronRight className="h-5 w-5 text-slate-400 transition group-open:rotate-90" />
        </summary>
        <div className="grid gap-4 border-t border-slate-200 p-3 dark:border-slate-800 sm:p-4 lg:grid-cols-2">
          <ShiftDashboardCard />
          <TimerCard />
        </div>
      </details>
