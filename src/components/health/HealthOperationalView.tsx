...
import { isDemoHealthLog, isDemoHealthProfile } from '../../lib/demoData';
...
export const HealthOperationalView: React.FC = () => {
  ...
  const realHealthLogs = useMemo(() => healthLogs.filter((log) => !isDemoHealthLog(log)), [healthLogs]);
  const profileIsDemo = isDemoHealthProfile(healthProfile);
  const latest = [...realHealthLogs].sort((a, b) => b.date.localeCompare(a.date))[0];
  const demoCount = healthLogs.length - realHealthLogs.length + Number(profileIsDemo);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-3 pb-[calc(5rem+env(safe-area-inset-bottom,0px))] animate-fade-in">
      <OperationalModeHeader
        ...
        description={`Últimos datos reales y entrenamiento durante ${shiftInfo.phase === 'work' ? 'faena' : 'descanso'}.`}
      ...
      {demoCount > 0 && (
        <section className="rounded-2xl border border-amber-800/70 bg-amber-950/20 px-3 py-2.5 text-xs text-amber-200">
          <span className="font-black">Datos demo ocultos.</span> Los valores seed no se muestran como biometría personal.
        </section>
      )}
      ...
      <section className="rounded-2xl bg-slate-950 p-3 text-white">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Último registro real</p>
            <p className="mt-1 text-lg font-black">{latest?.date || 'Sin datos reales'}</p>
            <p className="mt-0.5 text-xs text-slate-400">Peso de perfil: {!profileIsDemo && healthProfile.weightKg ? `${healthProfile.weightKg} kg` : 'sin dato real'}</p>
          </div>
      ...
      <section className="grid grid-cols-2 gap-2 lg:grid-cols-4">
        ...
        {metric('Peso', latest?.weightKg !== undefined ? `${latest.weightKg} kg` : !profileIsDemo && healthProfile.weightKg ? `${healthProfile.weightKg} kg` : '—', <Weight className="h-4 w-4" />)}
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h2 className="text-sm font-black text-slate-950 dark:text-white">Entrenamiento</h2>
          </div>
        ...
        <div className="mt-2 space-y-2">
          {weeklyWorkouts.slice(0, 4).map((workout) => (
            <div key={workout.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-100 px-3 py-2 dark:border-slate-800">
              <div>
          ...
          {weeklyWorkouts.length === 0 && <p className="py-4 text-center text-xs text-slate-500">Sin entrenamientos reales registrados en los últimos 7 días.</p>}
        </div>
      </section>
    </div>
  );
