...
export const FinancesOperationalView: React.FC = () => {
  ...
  const summary = useMemo(() => {
    ...
    const debt = debts.reduce((sum, item) => sum + item.remainingAmount, 0);
  ...
  }, [accounts, debts, month, recurringTransactions, transactions]);
  ...
  const budgetWarnings = useMemo(() => budgets
    ...
    .sort((a, b) => b.pct - a.pct), [budgets, month, transactions]);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 pb-[calc(5rem+env(safe-area-inset-bottom,0px))] animate-fade-in">
      <OperationalModeHeader
        eyebrow="Finanzas · modo operativo"
        title="Estado financiero actual"
        description="Primero caja, flujo del mes y alertas. Cuentas, deudas, presupuestos, metas y gráficos siguen disponibles en la vista completa."
      ...
      <section className="grid grid-cols-2 gap-2 lg:grid-cols-4">
        <div className="col-span-2 rounded-3xl bg-slate-950 p-4 text-white lg:col-span-1">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Balance total</p>
          <p className="mt-2 text-2xl font-black">{formatCLP(summary.balance)}</p>
          <p className="mt-1 text-[10px] text-slate-400">{accounts.length} cuenta{accounts.length === 1 ? '' : 's'}</p>
        </div>
        <div className="rounded-3xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          <ArrowUpRight className="h-4 w-4 text-emerald-500" />
          <p className="mt-2 text-[10px] font-black uppercase text-slate-400">Ingresos mes</p>
          <p className="mt-1 text-lg font-black text-slate-950 dark:text-white">{formatCLP(summary.income)}</p>
        </div>
        <div className="rounded-3xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          <ArrowDownRight className="h-4 w-4 text-rose-500" />
          <p className="mt-2 text-[10px] font-black uppercase text-slate-400">Gastos mes</p>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-2 text-[10px] font-black uppercase text-slate-400">Flujo neto</p>
      ...
      {(budgetWarnings.length > 0 || summary.debt > 0) && (
        <section className="rounded-3xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900 dark:bg-amber-950/20">
          ...
          <div className="mt-3 space-y-2 text-xs text-slate-700 dark:text-slate-300">
            ...
            {summary.debt > 0 && (
              <div className="flex items-center justify-between gap-3 border-t border-amber-200 pt-2 dark:border-amber-900">
                <span className="font-bold">Deuda pendiente total</span>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-sm font-black text-slate-950 dark:text-white">Últimos movimientos</h2>
          </div>
        ...
        <div className="mt-3 divide-y divide-slate-100 dark:divide-slate-800">
          {recent.length === 0 && <p className="py-6 text-center text-xs text-slate-500">Sin movimientos registrados.</p>}
          {recent.map((tx) => (
